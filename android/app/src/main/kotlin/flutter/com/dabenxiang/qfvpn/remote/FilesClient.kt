...
import flutter.com.dabenxiang.qfvpn.util.copyContentTo
...
class FilesClient(private val context: Context) {
    ...
    fun buildRootUri(): Uri{
        return DC.buildRootsUri(Authorities.FILES_PROVIDER)
    }

    suspend fun listDoc(documentId: String): List<File> = withContext(Dispatchers.IO) {
        val uri = DC.buildDocumentUri(Authorities.FILES_PROVIDER, documentId)
